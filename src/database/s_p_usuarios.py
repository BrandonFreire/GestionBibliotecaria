...
class SP_Usuarios:
    ...
    '''
    def consultar_usuarios_por_biblioteca(self,
                                          id_biblioteca: str,
                                          node: str = "FIS") -> List[Dict[str, Any]]:
        """
        Consulta usuarios de una biblioteca específica.
        ...
        Returns:
            Lista de usuarios de la biblioteca especificada.
        """
        query = "SELECT * FROM v_Usuario WHERE id_biblioteca = ?"
        try:
            return self.dist_conn.execute_query(node, query, (id_biblioteca,))
        except Exception as e:
            print(f"Error al consultar usuarios por biblioteca: {e}")
            return []
    ...
    def buscar_usuario_por_email(self,
        ...
        Args:
            email: Email a buscar (puede ser parcial).
            node: Nodo donde ejecutar (por defecto FIS).
        
        Returns:
            Lista de usuarios que coinciden con el email.
        """
        query = "SELECT * FROM v_Usuario WHERE email_usuario LIKE ?"
        try:
            return self.dist_conn.execute_query(node, query, (f"%{email}%",))
        except Exception as e:
